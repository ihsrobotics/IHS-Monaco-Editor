...
import { FileTabContext, FileTabValueContext, ReloadEditorContext, ToastContext, userSettingsContext } from "../App";
...
export default function DraggableTabsList({ onDragEnd }: Props) {
  ...
  const { setToastProps } = React.useContext(ToastContext);

  const { tabs } = React.useContext(FileTabContext);
  const { fileTabValue, setFileTabValue } = React.useContext(FileTabValueContext)
  const {reloadEditorFlag, setReloadEditorFlag} = React.useContext(ReloadEditorContext);
  ...
  const handleClickClose = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, index: number) => {
    e.stopPropagation();
    if(!tabs[index].editorSaved && saveFileOnEditorClose){
      console.log("save")
      saveFile(tabs[index].value, tabs[index].editorContent, setToastProps);
    }
      ...
      {tabs.map((tab, index) => {
        ...
        return (
          <DraggableTab
            ...
            <>
              <span>{tab.label}</span> 
              <IconButton aria-label="close" sx={{ height: "20px", width: "20px"}} onClick={(e) => handleClickClose(e, index)}> 
            ...
            index={index}
            key={index}
            child={child as React.DetailedReactHTMLElement<any, HTMLElement>}
          />
        );
      })}
