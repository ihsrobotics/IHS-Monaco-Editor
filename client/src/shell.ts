...
export function rename(oldName: string, loadFiles: () => void, toast: ToastFunction){
...
export function deleteItem(name: string, loadFiles: () => void, toast: ToastFunction){
...
export async function newFile(path: string, loadFiles?: () => void, toast?: ToastFunction, newFileName?: string){
...
export async function newFolder(parentDirName: string, loadFiles?: () => void, toast?: ToastFunction, newFolderName?: string){
        ...
        response => {
          if(toast){
            if(response.status == 200 || response.status == 204)
              toast(true, 'success', fileName.split('/').slice(-1).toString()+' saved successfully')
            else
              toast(true, 'warning', 'status '+response.status)
          }
          if(setIsFinished)
...
export async function command(command: string, toast?: ToastFunction, successMessage?: string){   
    try{
        ...
        if(toast && successMessage){
          if(response.status == 200)
            toast(true, 'success', successMessage)
          else
            toast(true, 'warning', 'status '+response.status+' '+response.json())
        }
...
function readShellOutput(value: Uint8Array | undefined, setPID: React.Dispatch<number>, oldTerminalOutput: [string, string, string][], newTerminalOutput: [string, string, string], setOutput: React.Dispatch<[string, string, string][]>, setCurrentDir: React.Dispatch<string>, ret: string){
...
export async function liveShell(command: string, cwd: string, setPID: React.Dispatch<number>, setIsFinished: React.Dispatch<boolean>, oldTerminalOutput: [string, string, string][], newTerminalOutput: [string, string, string], setOutput: React.Dispatch<[string, string, string][]>, setCurrentDir: React.Dispatch<string>){
    ...
    ret = readShellOutput(value, setPID, oldTerminalOutput, newTerminalOutput, setOutput, setCurrentDir, ret);
    while(!done){
        const { done, value } = await reader?.read() as ReadableStreamReadResult<Uint8Array>;
